...
// Plantilla original del Ministerio en base64
...
export default async function handler(req, res) {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    if (gs.length > 0) {
      ...
      ws.getCell(`C${fila}`).value = gs[0].horas !== null ? gs[0].horas : ''
    }
  }
  ...
  for (let i = 0; i < 15; i++) {
    ...
    if (gs.length > 0) {
      ...
      ws.getCell(`G${fila}`).value = gs[0].horas !== null ? gs[0].horas : ''
    }
  }
  ...
  const firmaUrl = ef.firma_url
  if (firmaUrl && firmaUrl.startsWith('data:image')) {
    try {
      const base64Firma = firmaUrl.split(',')[1]
  ...
  const filename = `Planilla_${ef.nombre.replace(/,/g,'').replace(/\s+/g,'_')}_${NOMBRE_MES.replace(' ','_')}.xlsx`

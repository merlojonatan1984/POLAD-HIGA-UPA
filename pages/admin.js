  ...
  return (
    <div>
      ...
      <div className="content">
        ...
        {vista === 'resumen' && (
          <div className="panel">
            <div className="panel-header"><h3>Estado por efectivo</h3></div>
            <div style={{ padding: 14, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: 8 }}>
              {efectivos.map(e => {
                ...
                return (
                  <div key={e.legajo} style={{ border: '0.5px solid rgba(0,0,0,0.1)', borderRadius: 8, padding: '10px 12px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <span className={`tag ${e.tipo === 'Uniformado' ? 'tag-u' : 'tag-g'}`} style={{ fontSize: 9, alignSelf: 'flex-start' }}>{e.tipo === 'Uniformado' ? 'U' : 'G'}</span>
                    </div>
                    <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 11, color: dias > 0 ? '#1D9E75' : '#BA7517' }}>{dias > 0 ? `${dias} días cargados` : 'Sin cargar'}</span>
                      <span style={{ fontSize: 12, fontWeight: 500, color }}>{hs} hs</span>
                    </div>

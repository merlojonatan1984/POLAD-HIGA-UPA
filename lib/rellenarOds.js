...
async function rellenarPlanillaOds(buffer, datos) {
  ...
  const escrituras = []
  escrituras.push([4, 3, datos.domicilio || '', 'string'])
  ...
  for (let i = 0; i < 16; i++) {
    ...
    if (g && g.length) {
      escrituras.push([fila, 1, g.map(x => x.horario).join(' y '), 'string'])
      escrituras.push([fila, 2, g.reduce((s, x) => s + (x.horas || 0), 0), 'float'])
    }
  }
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const g = datos.gdsMap[dia]
    if (g && g.length) {
      escrituras.push([fila, 4, g.map(x => x.horario).join(' y '), 'string'])
      escrituras.push([fila, 6, g.reduce((s, x) => s + (x.horas || 0), 0), 'float'])
    }
  }
